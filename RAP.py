...
def semantic_checks(tree, db):
  ...
  if tree.get_node_type() == 'times':
    ...
    for i,attr in enumerate(lattrs):
      if attr in rattrs:
        t_attrs.append(tree.get_left_child().get_relation_name() + "." + attr)
      else:
        t_attrs.append(attr)
    ...
    for i,attr in enumerate(rattrs):
      if attr in lattrs:
        t_attrs.append(tree.get_right_child().get_relation_name() + "." + attr)
      else:
        t_attrs.append(attr)
    ...
    for i,dom in enumerate(ldoms):
        if dom != rdoms[i]:
            return "SEMANTIC ERROR (UNION): " + lattrs[i] + " and " + rattrs[i] + \
                   " have different data types: " + ldoms[i] + " and " + rdoms[i]
...
def generateSQL(tree,db):
  if tree.get_node_type() == 'relation':
    ...
    for attr in tree.get_attributes():
      ...
      query += attr+", "
    query = query[:-2]
    query += " from ("+lquery+") "+tree.get_left_child().get_relation_name()
    return query
    ...
    if tree.get_left_child().get_node_type() == "union":
      lquery = "("+lquery+")"
    query = "select * from (("+lquery+") "+tree.get_left_child().get_relation_name()+") where "
    ...
    for attr in tree.get_attributes():
      query += attr+", "
    query = query[:-2] + ") in "
    query += "(select * from ("+rquery+") "+tree.get_right_child().get_relation_name()
    query += ")"
    return query
  else: # must be minus
    lquery = generateSQL(tree.get_left_child(),db)
